...
(function($app) {
  ...
  app.directive('asDate', maskDirectiveAsDate)
      ...
      .directive('crnListOrdered', function ($compile, $sce, $parse) {
        'use strict';
        return {
          restrict: 'EA',
          link: function (scope, element, attrs, ngModelCtrl) {
            ...
            switch (listType) {
              case 'numbered-list':
                templateDyn = `<ol style="list-style-type: ${attributeValue}">${content}</ol>`
                break

              case 'ordered-list':
                templateDyn = `<ul style="list-style-type: ${orderedAttributeValue}">${content}</ul>`
                break
            }
            element.html(templateDyn);
            $compile(templateDyn)(element.scope());    
